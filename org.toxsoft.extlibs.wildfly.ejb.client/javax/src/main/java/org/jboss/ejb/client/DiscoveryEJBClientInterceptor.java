...
public final class DiscoveryEJBClientInterceptor implements EJBClientInterceptor {
    ...
    static void addBlackListedDestination(URI destination) {
        if (destination != null) {
            // s5: 2022-02-03 mvk. TODO: требуется проверить необходимость нижестоящего патча после перехода на jboss-ejb-client-4.0.40
            // s5: mvk ??? проанализировать. Иногда сервер неверно(с точки зрения клиента) отвечает, за что попадает в
            // черный список
            // if( true ) {
            //    Logs.INVOCATION.warnf( "REJECT try Blacklisting destination = %s", destination );
            //    return;
            // }
            if (Logs.INVOCATION.isDebugEnabled()) {
