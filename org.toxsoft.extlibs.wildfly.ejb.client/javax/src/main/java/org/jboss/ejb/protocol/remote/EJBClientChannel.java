            ...
            switch (msg) {
                    ...
                    for (int i = 0; i < clusterCount; i ++) {
                        ...
                        for (int j = 0; j < memberCount; j ++) {
                            ...
                            for (int k = 0; k < mappingCount; k ++) {
                                ...
                                // s5: mvk finish original
                                final String destHost = message.readUTF();
                                final int destPort = message.readUnsignedShort();
                                if( S5ClusterNodeUtils.availableNode( destHost, destPort, 1000 ) == false ) {
                                   // Нет локального доступа к узлу кластера с указанным адресом и портом (проблема cluster -b 0.0.0.0)
                                   continue;
                                }
                                final CidrAddress block = CidrAddress.create( sourceIpBytes, netmaskBits );
                                // s5: mvk finish replace
                        ...
                        // s5: mvk
                        S5ClusterNodeUtils.onTopologyUpdated( msg, clusterName, addedNodes );
                    }
                    finishPart(0b10);
                    break;
                }
                case Protocol.CLUSTER_TOPOLOGY_REMOVAL: {
                    ...
                    for (int i = 0; i < clusterCount; i ++) {
                        ...
                        // s5: mvk
                        S5ClusterNodeUtils.onTopologyUpdated( msg, clusterName, new LinkedList<>() );
                ...
                case Protocol.CLUSTER_TOPOLOGY_NODE_REMOVAL: {
                    ...
                    for (int i = 0; i < clusterCount; i ++) {
                        String clusterName = message.readUTF();
                        // s5: mvk
                        List<String> removeNodes = new LinkedList<>();
                        int memberCount = PackedInteger.readPackedInteger(message);
                        for (int j = 0; j < memberCount; j ++) {
                        ...
                        // s5: mvk
                        S5ClusterNodeUtils.onTopologyUpdated( msg, clusterName, removeNodes );
                    }
                    break;
                }
                default: {
            ...
            try {
                synchronized (this) {
                    ...
                    for (; ; ) {
                        ...
                        // s5: mvk
                        if( retries > 0 ) {
                           throw new EJBException( "s5: EJBClientChannel, removeInvocationResult, retries, wait( 10000)... " );
                        }
                        // s5: mvk
                        // wait();
                        wait( S5ClusterNodeUtils.TRY_LOCK_TIMEOUT_DEFAULT );
                        retries++;
                    }
                }
