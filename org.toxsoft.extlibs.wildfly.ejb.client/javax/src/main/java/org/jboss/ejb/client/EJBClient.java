...
public final class EJBClient {
    ...
    // s5: mvk
    public static <T> T createProxy3(final EJBLocator<T> locator, final Supplier<AuthenticationContext> authenticationContextSupplier) {
        Assert.checkNotNullParam("locator", locator);
        return locator.createProxyInstance(new EJBInvocationHandler<T>(locator, authenticationContextSupplier));
    }
    ...
    // s5: mvk
    // Special hook method for naming; let's replace this sometime soon.
    public static <T> Pair<SessionID, T> createSessionProxy2( final StatelessEJBLocator<T> statelessLocator,
        Supplier<AuthenticationContext> authenticationContextSupplier, NamingProvider namingProvider )
        throws Exception {
      final EJBClientContext clientContext = EJBClientContext.getCurrent();
      // this is the auth context to use just for the invocation
      final AuthenticationContext authenticationContext;
      ...
      final EJBSessionCreationInvocationContext context =
          clientContext.createSessionCreationInvocationContext( statelessLocator, authenticationContext );
      final StatefulEJBLocator<T> statefulLocator =
          clientContext.createSession( context, statelessLocator, namingProvider );
      ...
      return Pair.create( statefulLocator.getSessionId(), proxy );
    }
