...
class RemoteEJBReceiver extends EJBReceiver {
    ...
    protected SessionID createSession(final EJBReceiverSessionCreationContext context) throws Exception {
        ...
        try {
            // s5: mvk start
            URI destination = context.getClientInvocationContext().getDestination();
            final IoFuture<ConnectionPeerIdentity> future =
               Endpoint.getCurrent().getConnectedIdentityIfExists( destination, "ejb", "jboss", authenticationContext );
            if( future != null ) {
                final ConnectionPeerIdentity identity = future.getInterruptibly();
                Connection connection = identity.getConnection();
                connection.close();
            }      
            // s5: mvk end
